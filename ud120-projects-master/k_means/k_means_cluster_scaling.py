...
def featureScaling_minmax(nparr):
    ...
    for lst in nparr:
        print lst[0]
        rescaled_nparr.append(scaler.fit_transform(lst.astype(float)))
    return rescaled_nparr
...
# feature scaling
from sklearn.preprocessing import MinMaxScaler
salary_s = [min(salary), 200000.0, max(salary)]
...
try:
    Draw(pred, finance_features, poi, mark_poi=False, name="clusters_scaling.pdf", f1_name=feature_1, f2_name=feature_2)
except NameError:
